...
#prepMenuOption
if ($install) {
  $prepMenuOption = "Install"
} elseif ($uninstall) {
  $prepMenuOption = "UnInstall"
} elseif ($help) {
  $prepMenuOption = "Help"
} else {
  $prepMenuOption = $null
...
Function ShowInfo {
  ...
  if ($WebHelp -like "*$verificationHeader*") {
    ...
    write-host ""
    write-host "  install: Installs a client (Specify '-client' flag and optionaly the '-startLauncher' flag)"
    write-host ""
...
while ($MainUI) {
  cls
  if ($prepMenuOption) {
    if ($hasAutoInstalled -eq $true) {
      $menuOption = "Exit"
    } else {
      $menuOption = "$prepMenuOption"
